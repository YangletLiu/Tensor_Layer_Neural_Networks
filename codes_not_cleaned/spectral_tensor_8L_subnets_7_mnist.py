...
#         # show_mnist_fig(x[0, 0], "original_image.png")
#         x = torch.split(x, split_size_or_sections=seg_length, dim=-1)
#         x = torch.cat(x, dim=-2)
#         # show_mnist_fig(x[0, 0], "split_image.png")
#     x = dct(x)
#     return x
...
def test_fusing_nets(epoch, nets, best_acc, best_fusing_acc, test_acc_list, fusing_test_acc_list, test_loss_list, fusing_test_loss_list, fusing_weight=None):
    ...
    with torch.no_grad():
        for batch_idx, (img, targets) in enumerate(testloader):
            ...
            img = preprocess_mnist(img, seg_length=7)
        ...
        print("\n| Validation Epoch #%d\t\t"%(epoch+1)
              ...
              +"%.4f, %.4f, %.4f] "%(test_loss[4], test_loss[5], test_loss[6])
              +" Acc: [%.2f%%, %.2f%%, %.2f%%, %.2f%%, "%(acc[0], acc[1], acc[2], acc[3])
              +"%.2f%%, %.2f%%, %.2f%%] "%(acc[4], acc[5], acc[6])
            )
    ...
    try:
        for epoch in range(num_epochs):
            ...
            for batch_idx, (inputs, targets) in enumerate(trainloader):
                ...
                inputs = preprocess_mnist(inputs, seg_length=7)
                for i in range(num_nets):
            ...
            print("| Best Acc: [%.2f%%, %.2f%%, %.2f%%, %.2f%%, "%(best_acc[0], best_acc[1], best_acc[2], best_acc[3])
                    +"%.2f%%, %.2f%%, %.2f%%]"%(best_acc[4], best_acc[5], best_acc[6])
                )
