...
def test_fusing_nets(epoch, nets, best_acc, best_fusing_acc, test_acc_list, fusing_test_acc_list, test_loss_list, fusing_test_loss_list, fusing_weight=None):
    ...
    with torch.no_grad():
        for batch_idx, (img, targets) in enumerate(testloader):
            ...
            img = preprocess_cifar10(img, seg_length=8)
        ...
        print("\n| Validation Epoch #%d\t\tLoss: [%.4f, %.4f, %.4f, %.4f, %.4f, %.4f, %.4f, %.4f] Acc: [%.2f%%, %.2f%%, %.2f%%, %.2f%%, %.2f%%, %.2f%%, %.2f%%, %.2f%%]   " 
              %(epoch+1, test_loss[0], test_loss[1], test_loss[2], test_loss[3], test_loss[4], test_loss[5], test_loss[6], test_loss[7],
...
def train_multi_nets(num_epochs, nets):
    ...
    try:
        for epoch in range(num_epochs):
            ...
            print('\n=> Training Epoch #%d, LR=[%.4f, %.4f, %.4f, %.4f, %.4f, %.4f, %.4f, %.4f]' %(
                   epoch+1, current_lr[0], current_lr[1], current_lr[2], current_lr[3],
                   current_lr[4], current_lr[5], current_lr[6], current_lr[7]))
            for batch_idx, (inputs, targets) in enumerate(trainloader):
                inputs, targets = inputs.to(device), targets.to(device) # GPU settings
                inputs = preprocess_cifar10(inputs, seg_length=8)
                for i in range(num_nets):
...
def save_record_and_draw(train_loss, train_acc, test_loss, test_acc, fusing_test_loss, fusing_test_acc):
    ...
    # write csv
    with open('block_spectral_conv_tensor_8L_subnets_8_cifar10_testloss.csv','w',newline='',encoding='utf-8') as f:
        f_csv = csv.writer(f)
    ...
    plt.sca(sub1)
    plt.title('block-spectral-conv-tensor-8L-subnets-8 Loss on CIFAR10 ')
    for i in range(num_nets):
    ...
    plt.sca(sub2)
    plt.title('block-spectral-conv-tensor-8L-subnets-8 Accuracy on CIFAR10 ')
    for i in range(num_nets):
    ...
    plt.savefig('./block_spectral_conv_tensor_8L_subnets_8_cifar10_trainloss.jpg')
