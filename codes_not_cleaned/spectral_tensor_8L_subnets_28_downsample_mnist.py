...
def train_multi_nets(num_epochs, nets):
    ...
    os.makedirs("./spec_models", exist_ok=True)
    for i in range(num_nets):
        torch.save(nets[i], "./block_spectral_tensor_8_no_noise_{}.pth".format(i))
...
def save_record_and_draw(train_loss, train_acc, test_loss, test_acc, fusing_test_loss, fusing_test_acc):
    # write csv
    with open('block_spectral_tensor_8L_mnist_subnets_28_no_noise_testloss.csv','w',newline='',encoding='utf-8') as f:
        f_csv = csv.writer(f)
    ...
    plt.sca(sub1)
    plt.title('block-spectral-tensor-8L-subnets-28 Loss on MNIST ')
    for i in range(num_nets):
    ...
    plt.sca(sub2)
    plt.title('block-spectral-tensor-8L-subnets-28 Accuracy on MNIST ')
    for i in range(num_nets):
    ...
    plt.savefig('./block_spectral_tensor_8L_subnets_28_mnist_no_noise.jpg')
