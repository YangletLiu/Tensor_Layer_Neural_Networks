...
def test(epoch, net, criterion, best_acc, test_acc_list, test_loss_list):
    ...
    with torch.no_grad():
        ...
        if acc > best_acc:
            ...
            torch.save(checkpoint, "fc_8L_mnist.pth.tar")
...
def save_record_and_draw(train_loss, train_acc, test_loss, test_acc):
    # write csv
    with open("fc_8L_mnist_testloss.csv", "w", newline='', encoding="utf-8") as f:
        f_csv = csv.writer(f)
    ...
    plt.sca(sub2)
    plt.title("FC-8L Accuracy on MNIST ")
