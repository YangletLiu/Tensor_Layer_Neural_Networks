...
def preprocess_mnist(img, seg_length=0):
    if seg_length:
        ...
        sp = img.shape
        x = img.reshape(sp[0], sp[1], -1, seg_length)
...
def test_fusing_nets(epoch, nets, best_acc, best_fusing_acc, test_acc_list, fusing_test_acc_list, test_loss_list, fusing_test_loss_list, fusing_weight=None):
    ...
    with torch.no_grad():
        for batch_idx, (img, targets) in enumerate(testloader):
            ...
            img = preprocess_mnist(img, seg_length=28)
...
def train_multi_nets(num_epochs, nets):
    ...
    try:
        for epoch in range(num_epochs):
            ...
            for batch_idx, (inputs, targets) in enumerate(trainloader):
                ...
                inputs = preprocess_mnist(inputs, seg_length=28)
    ...
    for i in range(num_nets):
        torch.save(nets[i], "./block_fc_8_no_noise_{}.pth".format(i))
...
def save_record_and_draw(train_loss, train_acc, test_loss, test_acc, fusing_test_loss, fusing_test_acc):
    # write csv
    with open('block_fc_8L_mnist_subnets_28_no_noise_testloss.csv','w',newline='',encoding='utf-8') as f:
        f_csv = csv.writer(f)
