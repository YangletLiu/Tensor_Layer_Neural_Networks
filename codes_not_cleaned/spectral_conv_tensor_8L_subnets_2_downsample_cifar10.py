        ...
        if seg_length == 2:
            ...
            for row in range(row_step):
                for col in range(col_step):
                    components.append(x[:, :, row::row_step, col::col_step].unsqueeze(dim=-1))
...
def test_fusing_nets(epoch, nets, best_acc, best_fusing_acc, test_acc_list, fusing_test_acc_list, test_loss_list, fusing_test_loss_list, fusing_weight=None):
    ...
    with torch.no_grad():
        for batch_idx, (img, targets) in enumerate(testloader):
            ...
            img = preprocess_cifar10(img, seg_length=2)
        ...
        print("\n| Validation Epoch #%d\t\tLoss: [%.4f, %.4f] Acc: [%.2f%%, %.2f%%]   " 
              %(epoch+1, test_loss[0], test_loss[1], acc[0], acc[1]))
...
def train_multi_nets(num_epochs, nets):
    ...
    try:
        for epoch in range(num_epochs):
            ...
            print('\n=> Training Epoch #%d, LR=[%.4f, %.4f]' %(epoch+1, current_lr[0], current_lr[1]))
            for batch_idx, (inputs, targets) in enumerate(trainloader):
                inputs, targets = inputs.to(device), targets.to(device) # GPU settings
                inputs = preprocess_cifar10(inputs, seg_length=2)
                for i in range(num_nets):
                ...
                sys.stdout.write('\r')
                sys.stdout.write('| Epoch [%3d/%3d] Iter[%3d/%3d]\tLoss: [%.4f, %.4f] Acc: [%.3f%%, %.3f%%]   '
                        %(epoch+1, num_epochs, batch_idx+1,
                          math.ceil(len(trainset)/batch_size), loss[0].item(), loss[1].item(),
                          100.*correct[0]/total[0], 100.*correct[1]/total[1]))
                sys.stdout.flush()
            ...
            now_time = time.time()
            print("| Best Acc: [%.2f%%, %.2f%%] "%(best_acc[0], best_acc[1]))
    ...
    print("\nBest training accuracy overall: [%.3f%%, %.3f%%] "%(best_acc[0], best_acc[1]))
...
def save_record_and_draw(train_loss, train_acc, test_loss, test_acc, fusing_test_loss, fusing_test_acc):
    ...
    # write csv
    with open('block_spectral_conv_tensor_8L_subnets_2_cifar10_testloss.csv','w',newline='',encoding='utf-8') as f:
        f_csv = csv.writer(f)
    ...
    plt.sca(sub1)
    plt.title('block-spectral-conv-tensor-8L-subnets-2 Loss on CIFAR10 ')
    for i in range(num_nets):
    ...
    plt.sca(sub2)
    plt.title('block-spectral-conv-tensor-8L-subnets-2 Accuracy on CIFAR10 ')
    for i in range(num_nets):
    ...
    plt.savefig('./block_spectral_conv_tensor_8L_subnets_2_cifar10_trainloss.jpg')
