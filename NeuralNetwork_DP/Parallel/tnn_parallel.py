...
def torch_tensor_product(tensorA, tensorB):
    ...
    if a_l == b_l and a_n == b_n:
        ...
        print('Shape Error')
    """
    tensorC = torch.zeros(a_l,a_m,b_p)
    for num in range(0,a_l-1):
        tensorC[num,:]=torch.mm(tensorA[num,:], tensorB[num,:])
    return tensorC
    """
...
def tnn_4(dataset, net_layers, batch_size, lr_rate, epochs_num):
    if dataset == 'MNIST':
    ...
    class tNN(nn.Module):
        def __init__(self):
            ...
            if net_layers == 4:
                ...
                self.W_3 = nn.Parameter(torch.randn((28, 28, 28), requires_grad=True, dtype=torch.float))
                self.B_3 = nn.Parameter(torch.randn((28, 28, 1), requires_grad=True, dtype=torch.float))
                self.W_4 = nn.Parameter(torch.randn((28, 28, 28), requires_grad=True, dtype=torch.float))
                self.B_4 = nn.Parameter(torch.randn((28, 28, 1), requires_grad=True, dtype=torch.float))
                self.W_5 = nn.Parameter(torch.randn((28, 28, 28), requires_grad=True, dtype=torch.float))
                self.B_5 = nn.Parameter(torch.randn((28, 28, 1), requires_grad=True, dtype=torch.float))
                self.W_6 = nn.Parameter(torch.randn((28, 28, 28), requires_grad=True, dtype=torch.float))
                self.B_6 = nn.Parameter(torch.randn((28, 28, 1), requires_grad=True, dtype=torch.float))
                self.W_7 = nn.Parameter(torch.randn((28, 28, 28), requires_grad=True, dtype=torch.float))
                self.B_7 = nn.Parameter(torch.randn((28, 28, 1), requires_grad=True, dtype=torch.float))
                self.W_8 = nn.Parameter(torch.randn((28, 10, 28), requires_grad=True, dtype=torch.float))
                self.B_8 = nn.Parameter(torch.randn((28, 10, 1), requires_grad=True, dtype=torch.float))
    ...
    return test_loss_epoch, test_acc_epoch, train_loss_epoch, train_acc_epoch, time_list
#test_loss, test_acc, train_loss, train_acc, time_list= tnn_4('MNIST',4,100,0.1,50)


def run_all(dataset, net_layers, batch_size, lr_rate, epochs_num):
    test_loss, test_acc, train_loss, train_acc, time_list = tnn_4(dataset, net_layers, batch_size, lr_rate, epochs_num)
    # write csv
    with open('tnn4_test.csv', 'w', newline='', encoding='utf-8') as f:
        f_csv = csv.writer(f)
    ...
    plt.savefig('./tnn4_parallel.jpg')
run_all('MNIST',4,100,0.1,50)
