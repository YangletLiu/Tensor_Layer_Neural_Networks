...
def save_record_and_draw(train_loss, train_acc, test_loss, test_acc):
    # write csv
    with open('de_fcn_8_mnist_testloss.csv','w',newline='',encoding='utf-8') as f:
        f_csv = csv.writer(f)
    ...
    plt.sca(sub1)
    plt.title('Decomposed FCN-8 Loss on MNIST ')
    ...
    plt.sca(sub2)
    plt.title('Decomposed FCN-8 Accuracy on MNIST ')
    ...
    plt.savefig('./de_fcn_8_mnist.jpg')
