...
class tNN8MNIST(nn.Module):
    def __init__(self):
        ...
        """
        std = 1
        self.W_1 = nn.Parameter(torch.randn(16, 49, 49) * std)
        self.B_1 = nn.Parameter(torch.randn(16, 49, 1) * std)
        self.W_2 = nn.Parameter(torch.randn(16, 49, 49) * std)
        self.B_2 = nn.Parameter(torch.randn(16, 49, 1) * std)
        self.W_3 = nn.Parameter(torch.randn(16, 49, 49) * std)
        self.B_3 = nn.Parameter(torch.randn(16, 49, 1) * std)
        self.W_4 = nn.Parameter(torch.randn(16, 49, 49) * std)
        self.B_4 = nn.Parameter(torch.randn(16, 49, 1) * std)
        self.W_5 = nn.Parameter(torch.randn(16, 49, 49) * std)
        self.B_5 = nn.Parameter(torch.randn(16, 49, 1) * std)
        self.W_6 = nn.Parameter(torch.randn(16, 49, 49) * std)
        self.B_6 = nn.Parameter(torch.randn(16, 49, 1) * std)
        self.W_7 = nn.Parameter(torch.randn(16, 49, 49) * std)
        self.B_7 = nn.Parameter(torch.randn(16, 49, 1) * std)
        self.W_8 = nn.Parameter(torch.randn(16, 10, 49) * std)
        self.B_8 = nn.Parameter(torch.randn(16, 10, 1) * std)
...
def save_record_and_draw(train_loss, train_acc, test_loss, test_acc):
    # write csv
    with open('tnn_8L_seg_16_mnist_testloss.csv','w',newline='',encoding='utf-8') as f:
        f_csv = csv.writer(f)
    ...
    plt.sca(sub1)
    plt.title('tNN-8L-seg-16 Loss on MNIST ')
    ...
    plt.sca(sub2)
    plt.title('tNN-8L-seg-16 Accuracy on MNIST ')
    ...
    plt.savefig('./tnn_8L_seg_16_mnist.jpg')
