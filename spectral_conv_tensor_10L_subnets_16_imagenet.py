...
batch_size = 1024
data_root = "/xfs/imagenet/"
trainset = datasets.ImageFolder(root=data_root+"train", transform=transform_train)
trainloader = torch.utils.data.DataLoader(trainset, batch_size=batch_size, shuffle=True, num_workers=4, pin_memory=True)
num_train = len(trainset)

testset = datasets.ImageFolder(root=data_root+"val", transform=transform_test)
...
save_models_path = "./dct_imagenet_models"
load_models_path = "./dct_imagenet_models"
...
def block_img(img, block_size):
    ...
    assert total_num_nets == row_blocks * col_blocks, "the number of downsampled images is not equal to the number of num_nets"
...
def test_fusing_nets(epoch, nets, best_acc, best_fusing_acc, test_acc_list, fusing_test_acc_list, test_loss_list, fusing_test_loss_list, fusing_weight=None, is_best=None):
    ...
    with torch.no_grad():
        for batch_idx, (img, targets) in enumerate(testloader):
            ...
            img = preprocess_imagenet(img, block_size=(4, 4))
    ...
    try:
        for epoch in range(num_epochs):
            ...
            for batch_idx, (inputs, targets) in enumerate(trainloader):
                ...
                inputs = preprocess_imagenet(inputs, block_size=(4, 4))
            ...
            best_acc, best_fusing_acc = test_fusing_nets(epoch, nets, best_acc, best_fusing_acc, test_acc_list,
                                        fusing_test_acc_list, test_loss_list, fusing_test_loss_list, fusing_weight=fusing_weight, is_best=is_best)
...
def save_record_and_draw(train_loss, train_acc, test_loss, test_acc, fusing_test_loss, fusing_test_acc):
    # write csv
    with open('spectral_conv_tensor_10L_imagenet_subnets_28_testloss.csv','w',newline='',encoding='utf-8') as f:
        f_csv = csv.writer(f)
    ...
    plt.sca(sub1)
    plt.title('spectral-conv-tensor-10L-subnets-28 Loss on ImageNet ')
    for i in range(num_nets):
    ...
    plt.sca(sub2)
    plt.title('spectral-conv-tensor-10L-subnets-28 Accuracy on ImageNet ')
    for i in range(num_nets):
    ...
    plt.savefig('./spectral_conv_tensor_10L_subnets_28_imagenet.jpg')
