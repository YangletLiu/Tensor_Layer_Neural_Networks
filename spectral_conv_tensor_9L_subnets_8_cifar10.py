...
def save_record_and_draw(train_loss, train_acc, test_loss, test_acc, fusing_test_loss, fusing_test_acc):
    ...
    # write csv
    with open('spectral_conv_tensor_8L_subnets_8_cifar10_testloss.csv','w',newline='',encoding='utf-8') as f:
        f_csv = csv.writer(f)
    ...
    plt.sca(sub1)
    plt.title('spectral-conv-tensor-8L-subnets-8 Loss on CIFAR10 ')
    for i in range(num_nets):
    ...
    plt.sca(sub2)
    plt.title('spectral-conv-tensor-8L-subnets-8 Accuracy on CIFAR10 ')
    for i in range(num_nets):
    ...
    plt.savefig('./spectral_conv_tensor_8L_subnets_8_cifar10_trainloss.jpg')
