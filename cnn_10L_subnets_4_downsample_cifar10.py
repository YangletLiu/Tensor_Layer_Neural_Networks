...
def save_record_and_draw(train_loss, train_acc, test_loss, test_acc, fusing_test_loss, fusing_test_acc):
    ...
    # write csv
    with open('block_cnn_8L_subnets_4_cifar10_testloss.csv','w',newline='',encoding='utf-8') as f:
        f_csv = csv.writer(f)
    ...
    plt.sca(sub1)
    plt.title('block-cnn-8L-subnets-4 Loss on CIFAR10 ')
    for i in range(num_nets):
    ...
    plt.sca(sub2)
    plt.title('block-cnn-8L-subnets-4 Accuracy on CIFAR10 ')
    for i in range(num_nets):
    ...
    plt.savefig('./block_cnn_8L_subnets_4_cifar10_trainloss.jpg')
