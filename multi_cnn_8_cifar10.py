...
def test_multi_nets(epoch, net, best_acc, test_acc_list, test_loss_list):
    for net in nets:
        net.eval()
    ...
    with torch.no_grad():
        ...
        print("\n| Validation Epoch #%d\t\t\tLoss: [%.4f, %.4f, %.4f] Acc: [%.2f%%, %.2f%%, %.2f%%]   " 
              %(epoch+1, np.mean(test_loss[0]), np.mean(test_loss[1]), np.mean(test_loss[2]), acc[0], acc[1], acc[2]))
...
def train_multi_nets(num_epochs, nets):
    ...
    try:
        for epoch in range(num_epochs):
            ...
            best_acc = test_multi_nets(epoch, net, best_acc, test_acc_list, test_loss_list)
...
def save_record_and_draw(train_loss, train_acc, test_loss, test_acc):
    ...
    for i in range(num_nets):
        with open('multi_cnn_8_cifar10_testloss.csv','w',newline='',encoding='utf-8') as f:
            ...
            if i == 0:
                f_csv.writerows(enumerate(test_loss[:, i],1))
            else:
                f_csv.writerows(test_loss[:, i])
            f_csv.writerow(['Train Loss:'])
            if i == 0:
                f_csv.writerows(enumerate(test_acc[:, i],1))
            else:
                f_csv.writerows(test_acc[:, i])
            if i == 0:
                f_csv.writerows(enumerate(train_loss[:, i],1))
            else:
                f_csv.writerows(train_loss[:, i])
            f_csv.writerow(['Test Acc:'])
            f_csv.writerow(['Train Acc:'])
            if i == 0:
                f_csv.writerows(enumerate(train_acc[:, i],1))
            else:
                f_csv.writerows(train_acc[:, i])
    ...
    for i in range(num_nets):
        plt.plot(np.arange(len(test_loss[:, i])), test_loss[:, i], color='red', label='TestLoss_{}'.format(i+1),linestyle='-')
        plt.plot(np.arange(len(train_loss[:, i])), train_loss[:, i], color='blue', label='TrainLoss'.format(i+1),linestyle='--')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.legend()
    ...
    for i in range(num_nets):
        plt.plot(np.arange(len(test_acc[:, i])), test_acc[:, i], color='green', label='TestAcc_{}'.format(i+1),linestyle='-')
        plt.plot(np.arange(len(train_acc[:, i])), train_acc[:, i], color='orange', label='TrainAcc_{}'.format(i+1),linestyle='--')
    plt.xlabel('Epoch')
    plt.ylabel('Accuracy(%)')
...
if __name__ == "__main__":
    ...
    for _ in range(num_nets):
        nets.append(build(decomp=False))
    print(nets[0])
    train_loss, train_acc, test_loss, test_acc = train_multi_nets(300, nets)
